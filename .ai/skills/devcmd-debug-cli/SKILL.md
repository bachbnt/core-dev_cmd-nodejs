...
6. Verify `./dev` and the relevant linked, packed, or GitHub-installed binary when installation behavior is involved.

## Diagnostic Commands

Use `dev doctor`, `dev inspect`, `dev recipes`, `command -v <tool>`, `node --version`, `npm --version`, and `npm pack --dry-run` as appropriate. Inspect structured executable and argument arrays instead of copying formatted display strings into a shell. DevCmd is installed from GitHub; never use the unrelated npm registry package named `devcmd` for installation tests.
